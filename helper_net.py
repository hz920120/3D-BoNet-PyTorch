...
class Ops:
    ...
    def gather_tensor_along_2nd_axis(bat_bb_pred, bat_bb_indices):
        bat_size = torch.Size(bat_bb_pred)[0]
        [_, ins_max_num, d1, d2] = bat_bb_pred.get_shape()
        bat_size_range = torch.arange(bat_size)
        bat_size_range_flat = torch.reshape(bat_size_range, [-1,1])
        bat_size_range_flat_repeat = torch.tile(bat_size_range_flat, [1, int(ins_max_num)])
        ...
        bat_bb_pred_new = torch.gather(bat_bb_pred, 0, indices_2d_flat_repeat)
    ...
    def hungarian(loss_matrix, bb_gt):
        ...
        ordering, loss_total = torch.py_func(assign_mappings_valid_only, [loss_matrix, bb_gt], [torch.int32, torch.float32])

        return ordering, loss_total

    @staticmethod
    def bbvert_association(X_pc, y_bbvert_pred, Y_bbvert, label=''):
        points_num = torch.Size(X_pc)[1]
        bbnum = int(y_bbvert_pred.shape[1])
        points_xyz = X_pc[:, :, 0:3]
        points_xyz = torch.tile(points_xyz[:, None, :, :], [1, bbnum, 1, 1])

        ##### get points hard mask in each gt bbox
        gt_bbox_min_xyz = Y_bbvert[:, :, 0, :]
        gt_bbox_max_xyz = Y_bbvert[:, :, 1, :]
        gt_bbox_min_xyz = torch.tile(gt_bbox_min_xyz[:, :, None, :], [1, 1, points_num, 1])
        gt_bbox_max_xyz = torch.tile(gt_bbox_max_xyz[:, :, None, :], [1, 1, points_num, 1])
        tp1_gt = gt_bbox_min_xyz - points_xyz
        tp2_gt = points_xyz - gt_bbox_max_xyz
        tp_gt = tp1_gt * tp2_gt
        points_in_gt_bbox_prob = cast(torch.equal(torch.mean(cast(torch.greater_equal(tp_gt, 0.), torch.float32), dim=-1), torch.tensor([1.0])), torch.float32)

        ##### get points soft mask in each pred bbox ---> Algorithm 1
        pred_bbox_min_xyz = y_bbvert_pred[:, :, 0, :]
        pred_bbox_max_xyz = y_bbvert_pred[:, :, 1, :]
        pred_bbox_min_xyz = torch.tile(pred_bbox_min_xyz[:, :, None, :], [1, 1, points_num, 1])
        pred_bbox_max_xyz = torch.tile(pred_bbox_max_xyz[:, :, None, :], [1, 1, points_num, 1])
        tp1_pred = pred_bbox_min_xyz - points_xyz
        tp2_pred = points_xyz - pred_bbox_max_xyz
        tp_pred = 100 * tp1_pred * tp2_pred
        tp_pred = torch.maximum(torch.minimum(tp_pred, 20.0), -20.0)
        points_in_pred_bbox_prob = 1.0/(1.0 + torch.exp(-1.0 * tp_pred))
        points_in_pred_bbox_prob = torch.min(points_in_pred_bbox_prob, dim=-1)

        ##### get bbox cross entropy scores
        prob_gt = torch.tile(points_in_gt_bbox_prob[:, :, None, :], [1, 1, bbnum, 1])
        prob_pred = torch.tile(points_in_pred_bbox_prob[:, None, :, :], [1, bbnum, 1, 1])
        ce_scores_matrix = - prob_gt * torch.log(prob_pred + 1e-8) - (1 - prob_gt) * torch.log(1 - prob_pred + 1e-8)
        ce_scores_matrix = torch.mean(ce_scores_matrix, axis=-1)
        ...
        ##### get bbox l2 scores
        l2_gt = torch.tile(Y_bbvert[:, :, None, :, :], [1, 1, bbnum, 1, 1])
        l2_pred = torch.tile(y_bbvert_pred[:, None, :, :, :], [1, bbnum, 1, 1, 1])
        l2_gt = torch.reshape(l2_gt, [-1, bbnum, bbnum, 2 * 3])
        l2_pred = torch.reshape(l2_pred, [-1, bbnum, bbnum, 2 * 3])
        l2_scores_matrix = torch.mean((l2_gt - l2_pred) ** 2, reduction_indices=[-1])
    ...
    def get_loss_psem_ce(y_psem_logits, Y_psem):
        psemce_loss = torch.nn.CrossEntropyLoss(y_psem_logits, Y_psem)
        # psemce_loss = torch.nn.softmax_cross_entropy_with_logits(logits=y_psem_logits, labels=Y_psem)
    ...
    def get_loss_bbvert(X_pc, y_bbvert_pred, Y_bbvert, label=''):
        ...
        points_xyz = X_pc[:, :, 0:3]
        points_xyz = torch.tile(points_xyz[:, None, :, :], [1, bb_num, 1, 1])
        ...
        Y_bbox_helper = cast(torch.greater(Y_bbox_helper, 0.), torch.float32)

        ##### 1. get ce loss of valid/positive bboxes, don't count the ce_loss of invalid/negative bboxes
        Y_bbox_helper_tp1 = torch.tile(Y_bbox_helper[:, :, None], [1, 1, points_num])
        ...
        bbox_pred_neg = torch.tile((1.- Y_bbox_helper)[:,:,None,None], [1,1,2,3])*y_bbvert_pred
    ...
    def get_loss_pmask(X_pc, y_pmask_pred, Y_pmask):
        ...
        Y_pmask_helper = cast(torch.greater(Y_pmask_helper, 0.), torch.float32)
