...
class Ops:
    ...
    def variable_sum(var, name):
        ...
        with tf.name_scope(name):
            ...
            torch.scalar_tensor('mean', mean)
            stddev = torch.sqrt(torch.mean(torch.square(var - mean)))
    ...
    def fc(x, out_d, name):
        ...
        in_d = x.get_shape()[1]
        with torch.device('cpu'):  # to create Variables stored on CPU memory
            # w = tf.get_variable(name + '_w', [in_d, out_d], initializer=xavier_init)
            w = Variable(name + '_w', [in_d, out_d])
            # b = tf.get_variable(name + '_b', [out_d], initializer=zero_init)
            b = Variable(name + '_b', [out_d])
