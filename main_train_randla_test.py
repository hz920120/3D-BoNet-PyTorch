...
from randla_net_s3dis import RandLA_Net_S3DIS as RandLA_new
...
if __name__ == '__main__':
    ...
    data_path = '/home/cesc/applications/workspace/s3dis_prepare'
    train_set = Data(data_path, train=True)
    ...
    for i, (points, neighbors, pools, up_samples, inputs, labels) in enumerate(train_loader):
        ...
        point_features, global_features, _, y_psem_logits = backbone(points, neighbors, pools, up_samples, inputs)  # B,num_cls,n_pts
...
def train_one_epoch(epoch, model, loss_fn, optimizer, train_loader, device, scheduler=None, writer=None):
    model.train()

    pbar = tqdm(enumerate(train_loader), total=len(train_loader))
    for step, (pc_xyzrgb, sem_labels, ins_labels, psem_onehot_labels, bbvert_padded_labels, pmask_padded_labels, _) in pbar:
        pc_xyzrgb = pc_xyzrgb.to(device) # B,N,12
        sem_labels = sem_labels.to(device) # B,N
        ins_labels = ins_labels.to(device) # B,N
        psem_onehot_labels = psem_onehot_labels.to(device) # B,N,13
        bbvert_padded_labels = bbvert_padded_labels.to(device) # B,24,2,3
        pmask_padded_labels = pmask_padded_labels.to(device) # B,24,N

        # with autocast(): # mixed-precision training
        optimizer.zero_grad()
        y_sem_pred, y_psem_logits, y_bbvert_pred_raw, y_bbscore_pred_raw, y_pmask_pred = model(pc_xyzrgb[:, :, 0:9], bbvert_padded_labels, is_train=True)
        # B,N,13. B,N,13. B,24,2,3. B,24. B,24,N

        # calculate loss
        loss, loss_psem_ce, loss_bbox, bbox_loss_l2, bbox_loss_ce, bbox_loss_iou, loss_bbox_score, loss_ins_pmask\
            = loss_fn(pc_xyzrgb, psem_onehot_labels, bbvert_padded_labels, pmask_padded_labels, y_psem_logits, y_bbvert_pred_raw, y_bbscore_pred_raw, y_pmask_pred)

        loss.backward()
        optimizer.step()
        optimizer.zero_grad()

        description = f'epoch {epoch} loss: {loss.cpu().detach().numpy():.4f} ' #psem: {loss_psem_ce:.4f} bbox: {loss_bbox:.4f} bbscore: {loss_bbox_score:.4f} pmask: {loss_ins_pmask:.4f}'
        pbar.set_description(description)
        if writer:
            writer.add_scalar('train_loss', loss, epoch * len(train_loader) + step)
            writer.add_scalar('psem_loss', loss_psem_ce, epoch * len(train_loader) + step)
            writer.add_scalar('bbox_loss', loss_bbox, epoch * len(train_loader) + step)
            writer.add_scalar('bbox_loss_l2', bbox_loss_l2, epoch * len(train_loader) + step)
            writer.add_scalar('bbox_loss_ce', bbox_loss_ce, epoch * len(train_loader) + step)
            writer.add_scalar('bbox_loss_iou', bbox_loss_iou, epoch * len(train_loader) + step)
            writer.add_scalar('bbscore_loss', loss_bbox_score, epoch * len(train_loader) + step)
            writer.add_scalar('pmask_loss', loss_ins_pmask, epoch * len(train_loader) + step)
