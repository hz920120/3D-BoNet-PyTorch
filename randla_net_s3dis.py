...
from utils.blocks import Dilated_res_block, random_sample, nearest_interpolation
from utils.s3dis_loader import Data_Configs_RandLA
...
class RandLA_Net_S3DIS(nn.Module):
    def __init__(self):
        ...
        self.num_cls = Data_Configs_RandLA.sem_num
        self.d_out = Data_Configs_RandLA.d_out  # [16, 64, 128, 256, 512]
        self.num_layers = Data_Configs_RandLA.num_layers
        self.n_pts = Data_Configs_RandLA.train_pts_num
        # [4, 4, 4, 4, 2]
        self.ratio = Data_Configs_RandLA.sub_sampling_ratio
        ...
        for i in range(self.num_layers):
            if index == 0:
                index = Data_Configs_RandLA.train_pts_num
            else:
                div_num *= self.ratio[i - 1]
                index += Data_Configs_RandLA.train_pts_num // div_num
        ...
        self.global_conv1 = pt_utils.Conv2d(self.d_out[4] * 2, self.d_out[4], kernel_size=(1, 1), bn=True)
        ...
        ##############  Decoder   #########
        self.point_conv1 = pt_utils.Conv2d(32, 128, kernel_size=(1, 1), bn=True)
    ...
    def forward(self, xyz, neigh_idx, sub_idx, interp_idx, x):
        ...
        global_features = self.global_conv1(sampled_4)
        ...
        point_features = torch.squeeze(self.point_conv1(decoder_4),
        ...
        y_sem_pred = F.log_softmax(x, dim=1)
        return point_features.transpose(1, 2), global_features, y_sem_pred.permute(0, 2, 1).contiguous(), x.permute(0, 2, 1).contiguous()
