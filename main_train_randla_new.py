...
if __name__ == '__main__':
    ...
    for ep in range(epoch + 1, epoch + 100, 1):
        ...
        for i, batchdata in pbar:
            for key in batchdata:
                if type(batchdata[key]) is list:
                    for j in range(len(batchdata[key])):
                        batchdata[key][j] = batchdata[key][j].to(device)
                else:
                    batchdata[key] = batchdata[key].to(device)
            ...
            if i % 20 == 0:
                ...
                print("-----------------------------------------------------------")
                x_axis = (ep+1) * Data_Configs_RandLA.n_pts
